...
class rk4_projectile_sequence extends uvm_sequence #(rk4_seq_item);
    ...
    static function void build_abs_half_prog(ref bit [15:0] mem [16]);
        ...
        mem[0]  = encode_instr(3'd0, 3'd0, 3'd5, 3'd7, 1'b0);  // ABS R0 -> R7
        mem[1]  = encode_instr(3'd7, 3'd0, 3'd4, 3'd7, 1'b0);  // SHR R7 -> R7
        for (int i = 2; i < 15; i++)
            mem[i] = encode_instr(3'd7, 3'd0, 3'd7, 3'd7, 1'b0);  // PASS R7 -> R7 (padding)
        mem[15] = encode_instr(3'd7, 3'd0, 3'd7, 3'd7, 1'b0);  // PASS R7 -> R7 (no halt)
    endfunction
